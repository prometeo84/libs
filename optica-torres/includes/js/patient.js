...
function readCity(provinceName, element) {
  ...
  $.ajax({
    ...
    data: {
      ...
      catalog_id: provinceName.value,
      provinceName: provinceName.selectedOptions[0].text,
    },
    cache: false,
    beforeSend: function (xhr) {
      if (provinceName.value == "") {
        alertPopUp(
          translate["advertice"],
          translate["catalogs_province_empty"],
          "warning",
        );
        xhr.abort();
      }
...
function savePatient() {
  ...
  var ophthalmological_family_history = $(
    "#a_ophthalmological_family_history",
  ).val();
...
function loadTableSearch(offset_pag, active_pag) {
  ...
  $.ajax({
    ...
    url: funcion,
    data: {
      function: "loadTableSearchPatients",
      limit: limite_pag,
      offset: offset_pag,
      active_p: active_pag,
